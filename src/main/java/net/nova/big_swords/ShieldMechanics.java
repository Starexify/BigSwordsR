...
public class ShieldMechanics {
  ...
  public static void register() {
    ServerLivingEntityEvents.AFTER_DAMAGE.register((entity, damageSource, baseDamageTaken, damageTaken, blocked) -> {
      if (!(entity instanceof Player player) || !player.isBlocking()) return;
      ...
      // Wooden Shields
      boolean isWooden = shield.is(BSItems.WOODEN_SHIELD.getFirst());
      boolean isGildedWooden = shield.is(BSItems.GILDED_WOODEN_SHIELD.getFirst());
      if (isWooden || isGildedWooden) {
        if (damageSource.is(DamageTypes.ARROW) && (sourceEntity instanceof AbstractArrow arrow && !(arrow instanceof ThrownTrident))) {
          ...
          // Weakness
          if (arrow.isOnFire())
            blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), blockedDamage.get() * 4);
        }
        ...
        if (fireAspectLevel > 0) {
          ...
          blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), blockedDamage.get() * multiplier);
        }
      ...
      // Stone Shields
      boolean isStone = shield.is(BSItems.STONE_SHIELD.getFirst());
      boolean isGildedStone = shield.is(BSItems.GILDED_STONE_SHIELD.getFirst());
      if (isStone || isGildedStone) {
        // Special
        if (attacker instanceof LivingEntity livingAttacker) {
          ItemStack weapon = livingAttacker.getMainHandItem();
          if (weapon.is(ItemTags.PICKAXES))
            blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), blockedDamage.get());
        }

        // Perk
        if (fireAspectLevel > 0 || soulFireAspectLevel > 0) playSound(level, player, SoundEvents.FIRE_EXTINGUISH);
        ...
        if ((damageSource.is(DamageTypes.EXPLOSION) || damageSource.is(DamageTypes.PLAYER_EXPLOSION))) {
          ...
          player.hurtServer(serverLevel, damageSource, damageToPlayer);
          blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), blockedDamage.get() + damageToPlayer);
        }
      }

      // Iron Shields Perk
      boolean isIron = shield.is(BSItems.IRON_SHIELD.getFirst());
      boolean isGildedIron = shield.is(BSItems.GILDED_IRON_SHIELD.getFirst());
      if ((isIron || isGildedIron) && (damageSource.is(DamageTypes.EXPLOSION) || damageSource.is(DamageTypes.PLAYER_EXPLOSION)))
        blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), isGildedIron ? 0 : blockedDamage.get() / 2);

      // Diamond Shields
      boolean isDiamond = shield.is(BSItems.DIAMOND_SHIELD.getFirst());
      boolean isGildedDiamond = shield.is(BSItems.GILDED_DIAMOND_SHIELD.getFirst());
      if (isDiamond || isGildedDiamond) {
        ...
        if (randomChance < reflectChance) {
          if (sourceEntity instanceof Projectile projectile && !(projectile instanceof ThrownTrident)) {
            handleReflection(level, player, attacker, projectile, blocksAttacks, shield, blockedDamage.get());
          }
        }
      }

      // Netherite Shields
      boolean isNetherite = shield.is(BSItems.NETHERITE_SHIELD.getFirst());
      boolean isGildedNetherite = shield.is(BSItems.GILDED_NETHERITE_SHIELD.getFirst());
      if (isNetherite || isGildedNetherite) {
        if (randomChance < 0.5 && attacker != null) {
          // Perk
          float damageToReflect = isGildedNetherite ? blockedDamage.get() * 0.5f : blockedDamage.get() * 0.3f;
          ...
          // Weakness
          if (randomChanceE < cooldownChance) blocksAttacks.disable(serverLevel, player, cooldownTime, shield);
        }
      }

      // Ender Shields
      boolean isEnder = shield.is(BSItems.ENDER_SHIELD.getFirst());
      boolean isGildedEnder = shield.is(BSItems.GILDED_ENDER_SHIELD.getFirst());
      ...
      // Quartz Shields
      boolean isQuartz = shield.is(BSItems.QUARTZ_SHIELD.getFirst());
      boolean isGildedQuartz = shield.is(BSItems.GILDED_QUARTZ_SHIELD.getFirst());
      ...
      // Patchwork Shields
      boolean isPatchwork = shield.is(BSItems.PATCHWORK_SHIELD.getFirst());
      boolean isGildedPatchwork = shield.is(BSItems.GILDED_PATCHWORK_SHIELD.getFirst());
      ...
      // Skull Shields
      boolean isSkull = shield.is(BSItems.SKULL_SHIELD.getFirst());
      boolean isGildedSkull = shield.is(BSItems.GILDED_SKULL_SHIELD.getFirst());
      if (isSkull || isGildedSkull) {
        ...
        if (randomChance < weaknessChance)
          blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), blockedDamage.get() * 3);
      }

      // Biomass Shields
      boolean isBiomass = shield.is(BSItems.BIOMASS_SHIELD.getFirst());
      boolean isGildedBiomass = shield.is(BSItems.GILDED_BIOMASS_SHIELD.getFirst());
      if (isBiomass || isGildedBiomass) {
        ...
        if (randomChanceE < 0.15) {
          ...
          blocksAttacks.hurtBlockingItem(level, shield, player, player.getUsedItemHand(), healthToDamage);
        }
      }

      // Livingmetal Shields
      boolean isLivingmetal = shield.is(BSItems.LIVINGMETAL_SHIELD.getFirst());
      boolean isGildedLivingmetal = shield.is(BSItems.GILDED_LIVINGMETAL_SHIELD.getFirst());
