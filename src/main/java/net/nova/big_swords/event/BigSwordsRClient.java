            ...
            ModList.get().getModContainerById(MODID).ifPresent(modContainer -> {
                event.addRepositorySource((packConsumer) -> {
                    ...
                    Pack.Metadata metadata = new Pack.Metadata(
                            Component.translatable(RP_16x_DESC), // Description
                            PackCompatibility.COMPATIBLE, // Compatibility level
                            FeatureFlagSet.of(), // Requested features (empty for now)
                            Collections.emptyList(), // Overlays (empty for now)
                            false // Is required
                    );
