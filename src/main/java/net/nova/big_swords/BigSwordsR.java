...
public class BigSwordsR implements ModInitializer {
  ...
  public void onInitialize() {
    ...
    // Loot Table Modifier
    LootTableEvents.MODIFY.register((resourceKey, builder, lootTableSource, _) -> {
      if (lootTableSource.isBuiltin() && BuiltInLootTables.END_CITY_TREASURE.equals(resourceKey)) {
        LootPool.Builder poolBuilder = LootPool.lootPool()
            .when(LootItemRandomChanceCondition.randomChance(0.35f))
            .add(LootItem.lootTableItem(BSItems.ENDER_UPGRADE_SMITHING_TEMPLATE.getFirst().value()));
    ...
    ServerEntityEvents.ENTITY_LOAD.register((entity, serverWorld) -> {
      if (entity.getType().equals(Tags.EntityTypeTags.HALLOWEEN_MOB) && entity instanceof Mob mob)
        halloweenDrop(mob, serverWorld, new ItemStack(BSItems.SOUL_REAPER.getFirst()));
    });
  }
  ...
  public static double getModifierValue(List<ItemAttributeModifiers.Entry> modifiers, Identifier modifierId) {
