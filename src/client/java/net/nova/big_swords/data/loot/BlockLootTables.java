...
public class BlockLootTables extends FabricBlockLootSubProvider {
  ...
  public void generate() {
    ...
    dropSelf(BSBlocks.LIVINGMETAL_BLOCK.getFirst().value());
    dropSelf(BSBlocks.BIOMASS_BLOCK.getFirst().value());

    // Creep Block Drops
    add(BSBlocks.CREEP_BLOCK.getFirst().value(), block -> this.createSingleItemTableWithSilkTouch(block, Blocks.SOUL_SAND));

    // Biomass Drops
    LootItemCondition.Builder ageCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(BSBlocks.BIOMASS.getFirst().value())
    ...
    add(BSBlocks.BIOMASS.getFirst().value(), createBiomassDrops(BSBlocks.BIOMASS.getFirst().value(), BSItems.BIOMASS.getFirst().value(), BSItems.BIOMASS_SEED.getFirst().value(), ageCondition, scytheCondition));
  }
